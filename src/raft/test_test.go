...
func TestManyElections2A(t *testing.T) {
	...
	first := cfg.checkOneLeader()
	DPrintf("old leader: %d\n", first)
	...
	for ii := 1; ii < iters; ii++ {
		...
		i2 := rand.Int() % servers
		i3 := rand.Int() % servers
		DPrintf("round %d : disconnect %d, %d, %d\n", ii, i1, i2, i3)
		cfg.disconnect(i1)
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		...
		second := cfg.checkOneLeader()
		DPrintf("round %d : new leader: %d\n", ii, second)

		DPrintf("round %d : connect %d, %d, %d\n", ii, i1, i2, i3)
		cfg.connect(i1)
		cfg.connect(i2)
		cfg.connect(i3)
	}
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("old leader disconnect %d", leader1)
	cfg.disconnect(leader1)

	// make old leader try to agree on some entries
	DPrintf("old leader %d agree on %d, %d, %d", leader1, 102, 103, 104)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("new leader disconnect %d", leader2)
	cfg.disconnect(leader2)

	// old leader connected again
	DPrintf("old leader %d reconnect", leader1)
	cfg.connect(leader1)

	cfg.one(104, 2, true)

	// all together now
	DPrintf("new leader %d reconnect ", leader2)
	cfg.connect(leader2)
...
func TestBackup2BSimply(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
	...
	DPrintf("reconnect %d, %d, %d", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	DPrintf("reconnect %d, %d, %d", (leader1+0)%servers, (leader1+1)%servers, other)
